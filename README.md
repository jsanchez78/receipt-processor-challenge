...
Open a UNIX terminal and run the following commands:
...
Now open a separate UNIX terminal & begin running request.
...
```bash
cd web-service
curl http://localhost:8087/receipts/process \                                      
    --include \
...
```bash
curl http://localhost:8087/receipt
{ID}/points
```
...
Run test cases to process receipts from the provided input.json file

```bash
go test -v
```

Will successfully run several unit test for the following rules:
